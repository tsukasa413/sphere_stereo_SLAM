...
class ISB_Filter:
    def __init__(self, candidate_count, resolution, device):
        ...
        # Read helper header and CUDA source. Use utf-8 and ignore errors so
        # stray non-UTF8 bytes or smart quotes in comments won't crash.
        with open('python/vec_utils.cuh', 'r', encoding='utf-8', errors='ignore') as f:
            utils_source = f.read()
        with open('python/isb_filter.cu', 'r', encoding='utf-8', errors='ignore') as f:
            cuda_source = utils_source + f.read()
        cuda_source = cuda_source.replace("CANDIDATE_COUNT", str(candidate_count))

        # Normalize and strip non-ascii characters that may cause
        # nvrtc/cupy to fail when writing the temporary CU file.
        # This removes smart quotes or other unicode punctuation from
        # comments or embedded text while keeping the code intact.
        cuda_source = unicodedata.normalize('NFKD', cuda_source)
        cuda_source = cuda_source.encode('ascii', 'ignore').decode('ascii')
