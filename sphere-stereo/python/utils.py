...
def parse_json_calib(raw_calibration, matching_resolution, device, original_resolution=None):
    """
    ...
    Args:
        ...
        original_resolution: [2] Original image resolution [width, height]. If None, will try to get from calibration file.
    """
    calibrations = []
    
    # Get resolution info
    if original_resolution is not None:
        resolutions = [original_resolution] * len(raw_calibration['T_imu_cam'])
    elif 'resolution' in raw_calibration:
        resolutions = raw_calibration['resolution']
    else:
        # Default resolution - you may want to adjust this based on your images
        default_resolution = [3840, 2160]  # Based on the actual image we found (width, height)
        resolutions = [default_resolution] * len(raw_calibration['T_imu_cam'])
        print(f"Warning: No resolution found in calibration, using default {default_resolution}")
    
    for extrinsics, intrinsics, original_resolution \
            in zip(raw_calibration['T_imu_cam'], raw_calibration['intrinsics'], resolutions):
...
def evaluate_rgbd_panorama(rgbd_panoramas, filename, dataset_path, bad_px_ratio_thresholds, panorama_resolution):
    ...
    try:
        ...
        gt_distance = cv2.imread(os.path.join(dataset_path, "gt/inv_distance_" + read_name + ".tif"), 
        ...
        if(gt_rgb is not None and gt_rgb is not None 
            ...
            # Calculate appropriate win_size based on image dimensions
            min_dim = min(gt_rgb.shape[0], gt_rgb.shape[1])
            win_size = min(7, min_dim if min_dim % 2 == 1 else min_dim - 1)
            win_size = max(3, win_size)  # Ensure minimum win_size of 3
            
            # Use channel_axis instead of multichannel (deprecated parameter)
            if len(gt_rgb.shape) == 3 and gt_rgb.shape[2] == 3:
                ssim = structural_similarity(gt_rgb, evaluated_rgb, win_size=win_size, channel_axis=2)
            else:
                ssim = structural_similarity(gt_rgb, evaluated_rgb, win_size=win_size)
