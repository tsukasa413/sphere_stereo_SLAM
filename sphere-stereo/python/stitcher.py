...
class Stitcher:
    def __init__(self, calibrations, reprojection_viewpoint, masks, min_dist, max_dist, 
        ...
        # Read helper header and CUDA source. Use utf-8 and ignore errors to
        # avoid UnicodeDecodeError when files contain non-ascii punctuation.
        with open('python/vec_utils.cuh', 'r', encoding='utf-8', errors='ignore') as f:
            utils_source = f.read()
        with open('python/stitcher.cu', 'r', encoding='utf-8', errors='ignore') as f:
        ...
        # Normalize and strip non-ascii characters to avoid nvrtc unicode
        # encoding errors when CuPy writes the temporary .cu file.
